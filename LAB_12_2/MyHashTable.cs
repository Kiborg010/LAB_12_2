...
namespace LAB_12_2
{
    public class MyHashTable<T> where T : IInit, ICloneable, new()
    {
        ...
        double fillRatio; //Коэффициент заполнения. При превышении соотношения количества и вместимости с этим коэффициентом, таблица будет увеличена вдвое.
        ...
        public int FindItem(T data) //Метод для поиска элемента в хэш-таблице
        {
            ...
            else if (tableNextFactor[index] == true) //Если этот фактор true, то значит есть смысл искать элемент дальше в таблице
            {
                ...
                while (current < tableValue.Length) //Пока текущий индекс не дошёл до конца таблицы
                {
                    if (data.Equals(tableValue[current])) //Если элемент под текущим индексом совпадает с искомой информацией
                    {
                        return current; //То возвращаем текущий индекс
                    }
                ...
                while (current < index) //Пока текущий индекс не дойдёт до изначального индекса
                {
                    if (data.Equals(tableValue[current])) //Если элемент под текущим индексом совпадает с искомой информацией
                    {
                        return current;//То возвращаем текущий индекс
            ...
            return -1; //Ну и самый последний вариант, когда вообще всё обыскали. Ничего не нашли и поняли, что нет такого элемента. Возвращаем -1, то есть показатель того, что элемента нет
